...
async function removeParticipant(id_user, id_espace) {

    if (!confirm("Êtes-vous sûr de vouloir supprimer ce participant ?")) return;

    const res = await API.removeParticipant(id_user, id_espace);

    if (res.status === 200) window.location.reload();
    else alert("Erreur lors de la suppression du participant");
}